...
<div class="wrapper">
  ...
  <div class="content-wrapper">
	...
	?>
			...
			{
			?>
            <form action="<?php echo BASE_URI.'backend/picture/addbanner'; ?>" method="post" enctype="multipart/form-data">
            <?php
			}
			else if($feature=='Edit')
			{
			?>
            <form action="<?php echo BASE_URI.'backend/picture/editbanner'; ?>" method="post" enctype="multipart/form-data">
            <?php
			}
			?>
            <input type="hidden" name="txtCid" value="<?php if(isset($bannerinfo['picture_id'])){echo $bannerinfo['picture_id'];} ?>"/>
          
            
              <div class="box-body">
                <div class="form-group">
                  <label for="exampleInputEmail1">Picture Name</label>
                  <input type="text" class="form-control" name="txtTitle" placeholder="Enter Category Name" value="<?php if(isset($bannerinfo['picture_title'])){echo $bannerinfo['picture_title'];} ?>" required>
                </div>
                 ...
                 <div class="form-group">
                  <label for="exampleInputFile">Description</label>
                   <textarea id="editor1" name="editor1" rows="10" cols="120" required>
                                           <?php if(isset($bannerinfo['picture_content'])){echo $bannerinfo['picture_content'];} ?>
                    </textarea>
                </div>
              </div>
              <!-- /.box-body -->
			...
			?>
                ...
                <button type="submit" class="btn btn-primary" value="new" name="btnAdd">Save and Add New</button>
                 <a class="btn btn-default" href="<?php echo BASE_URI.'backend/picture'; ?>">Back To List</a>
                  <?php
			}
			else if($feature=='Edit')
			{
