...
<div class="wrapper">
  ...
  <div class="content-wrapper">
	...
	?>
			...
			{
			?>
            <form action="<?php echo BASE_URI.'backend/review/addbanner'; ?>" method="post" enctype="multipart/form-data">
            <?php
			}
			else if($feature=='Edit')
			{
			?>
            <form action="<?php echo BASE_URI.'backend/review/editbanner'; ?>" method="post" enctype="multipart/form-data">
            <?php
			}
			?>
            <input type="hidden" name="txtCid" value="<?php if(isset($bannerinfo['review_id'])){echo $bannerinfo['review_id'];} ?>"/>
             <input type="hidden" name="hid_image" id="hid_image" value="<?php if(isset($bannerinfo['review_image'])){echo $bannerinfo['review_image'];} ?>" />
            
              <div class="box-body">
                <div class="form-group">
                  <label for="exampleInputEmail1">Username</label>
                  <input type="text" class="form-control" name="txtTitle" placeholder="Enter User Name" value="<?php if(isset($bannerinfo['review_user'])){echo $bannerinfo['review_user'];} ?>" required>
                </div>
                <div class="form-group">
                  <label for="exampleInputEmail1">Occupation</label>
                  <input type="text" class="form-control" name="txtPlace" placeholder="Enter Occupation" value="<?php if(isset($bannerinfo['review_occupation'])){echo $bannerinfo['review_occupation'];} ?>" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  <label for="exampleInputFile">Message</label>
                   <textarea id="editor1" name="editor1" rows="10" cols="120">
                                           <?php if(isset($bannerinfo['review_content'])){echo $bannerinfo['review_content'];} ?>
                    </textarea>
                </div>
			...
			?>
                ...
                <button type="submit" class="btn btn-primary" value="new" name="btnAdd">Save and Add New</button>
                 <a class="btn btn-default" href="<?php echo BASE_URI.'backend/review'; ?>">Back To List</a>
                  <?php
			}
			else if($feature=='Edit')
			{
