...
<div class="wrapper">
  ...
  <div class="content-wrapper">
	...
	?>
			...
			{
			?>
            <form action="<?php echo BASE_URI.'backend/video_banner/addbanner'; ?>" method="post" enctype="multipart/form-data">
            <?php
			}
			else if($feature=='Edit')
			{
			?>
            <form action="<?php echo BASE_URI.'backend/video_banner/editbanner'; ?>" method="post" enctype="multipart/form-data">
            <?php
			}
			?>
            <input type="hidden" name="txtCid" value="<?php if(isset($bannerinfo['video_banner_id'])){echo $bannerinfo['video_banner_id'];} ?>"/>
          
            
              <div class="box-body">
                <div class="form-group">
                  <label for="exampleInputEmail1">Title 1</label>
                  <input type="text" class="form-control" name="txtTitle" placeholder="Enter Category Name" value="<?php if(isset($bannerinfo['video_banner_title'])){echo $bannerinfo['video_banner_title'];} ?>" required>
                </div>
                 ...
                 <div class="form-group">
                  <label for="exampleInputFile">Content</label>
                   <textarea id="editor1" name="editor1" rows="10" cols="120" required>
                                           <?php if(isset($bannerinfo['video_banner_content'])){echo $bannerinfo['video_banner_content'];} ?>
                    </textarea>
                </div>
              </div>
              <!-- /.box-body -->
			...
			?>
                ...
                <button type="submit" class="btn btn-primary" value="new" name="btnAdd">Save and Add New</button>
                 <a class="btn btn-default" href="<?php echo BASE_URI.'backend/video_banner'; ?>">Back To List</a>
                  <?php
			}
			else if($feature=='Edit')
			{
